...
- Optional desktop LLM HTTP backends behind the `providers-http` feature: OpenAI Responses via `OPENAI_API_KEY` and Anthropic Messages via `ANTHROPIC_API_KEY`; normal builds keep network/provider calls out unless explicitly enabled
...
- Fake-transport coverage for the opt-in OpenAI and Anthropic LLM backends so request mapping, secret/header construction, response parsing, and transport errors are tested without real API calls
...
- Broader live Provider/API execution beyond the opt-in OpenAI/Anthropic LLM desktop paths
...
```bash
OPENAI_API_KEY=... ANTHROPIC_API_KEY=... dx serve --desktop --features desktop,providers-http
```

In the app, set OpenAI or Anthropic to the platform/env mode in Provider Settings and run an LLM node with provider `openai` or `anthropic`. Anthropic defaults to the Messages API with `claude-sonnet-4-6` when the workflow does not provide a concrete model. The regular desktop and web commands do not include live provider HTTP clients.

...
- Media capability profiles for image/audio/video/3D-capable nodes, with web/desktop readiness and adapter-gap summaries surfaced in the app sidebar; inline image compare metrics and split-grid PNG/JPEG/WebP transforms now have first Rust adapters, video frame-grab records source/seek metadata and exposes an opt-in WebView video/canvas capture action for renderable sources, GLB viewer records metadata and provides WebView model-viewer preview plus opt-in PNG snapshot capture for renderable GLB URIs without fake Run Local capture, split-grid can generate legacy child ImageInput/Prompt/Generate sets, and desktop project load hydrates saved image/model refs back into transformable data URLs
...
- Media storage polish: richer node-specific editor/player affordances, local/offline GLB model-viewer bundling, stronger clipboard fallbacks, and broader media transform adapters

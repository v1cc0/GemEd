...
- Node-card media preview foundation that detects common inline/reference media fields, renders size-guarded image/audio/video previews, exposes Open/Download links, and flags GLB/project-reference adapter gaps honestly
...
- Media storage polish: richer node-specific editor/lightbox/player affordances, GLB/WebGL previews, and real media transform adapters
...
To opt into the experimental desktop OpenAI LLM backend:

```bash
OPENAI_API_KEY=... dx serve --desktop --features desktop,providers-http
```

In the app, set OpenAI to the platform/env mode in Provider Settings and run an LLM node with provider `openai`. The regular desktop and web commands do not include live provider HTTP clients.
